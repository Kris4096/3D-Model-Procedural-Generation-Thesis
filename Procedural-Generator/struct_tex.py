...
def train_struct_tex_gen():
    input_data = [];
    training_data = [];
    combined_data = utils.glob_only_colored_data("dataset/structuretex",input_data, training_data);
    ...
    keras.utils.plot_model(encoder,to_file="model_struct_tex_encoder.png",show_shapes=True)
    ...
    keras.utils.plot_model(decoder,to_file="model_struct_tex_decoder.png",show_shapes=True)
    ...
    vae.fit(input_data, training_data, epochs=20, shuffle=True, validation_split=0.2, batch_size=64)

    vae.summary()
    vae.save("tensordata/StructTex")
    return vae

def load_struct_tex_gen():
    model = keras.models.load_model("tensordata/StructTex")
    return model