...
def train_flora_tex_gen():
    input_data = [];
    training_data = [];
    combined_data = utils.glob_only_colored_data("dataset/floratex",input_data, training_data);
    ...
    x = layers.Conv3D(24, conv_filter, padding='same')(original_inputs)
    ...
    keras.utils.plot_model(encoder,to_file="model_flora_tex_encoder.png",show_shapes=True)
    ...
    x = layers.UpSampling3D(pool_filter)(x)
    x = layers.Conv3D(24, conv_filter, padding='same', activation='relu')(x)
    ...
    keras.utils.plot_model(decoder,to_file="model_flora_tex_decoder.png",show_shapes=True)
    ...
    vae.fit(input_data, training_data, epochs=50, shuffle=True, validation_split=0.1, batch_size=8)

    vae.summary()
    vae.save("tensordata/FloraTex")
    return vae

def load_flora_tex_gen():
    model = keras.models.load_model("tensordata/FloraTex")
    return model